...
def test_rate_limiter_respects_limit():
    rate_limiter = RateLimiter(max_requests=15, window_seconds=1.0)
...
def test_rate_limiter_cleanup(mock_time):
    ...
    # Check rate limit to trigger cleanup
    assert rate_limiter.check_rate_limit() is True
    # Old requests should be cleaned up
    assert len(rate_limiter.request_timestamps) == 1  # Only the new request should remain
