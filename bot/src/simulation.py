...
def simulate_sandwich(
    ...
    # Normalize amounts for comparison (convert SOL's 9 decimals to USDC's 6 decimals)
    normalized_amount = amount_in
    if amount_in > amount_out:  # Buy order (SOL -> USDC)
        normalized_amount = amount_in // 1000  # Convert from 9 to 6 decimals
        
    # Return None for small transactions (1 USDC or less)
    if normalized_amount <= 1_000_000:  # 1 USDC threshold
        return None
        
    # Return simulation result for valid trades
