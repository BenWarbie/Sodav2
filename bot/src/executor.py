    ...
    async def execute_sandwich(
        ...
        try:
            if dry_run:
                ...
                front_sig = "DRY_RUN_FRONT_%d" % timestamp
                back_sig = "DRY_RUN_BACK_%d" % timestamp

                # Log simulated execution
                logger.info(
                    "Simulated transaction (not valid on Explorer): %s", front_sig
                )
                logger.info(
                    "Simulated transaction (not valid on Explorer): %s", back_sig
                )
                logger.info(
                    "NOTE: These are simulated transactions and cannot be "
                    "viewed on Explorer"
                )
            ...
            logger.error("Failed to execute sandwich attack: %s", e)
            return None

    async def _send_transaction(
        self, transaction: Transaction, max_retries: int, initial_backoff: float = 1.0
    ) -> Optional[str]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt > 0:
                    ...
                    logger.info(
                        "Retrying transaction in %.2f seconds " "(attempt %d/%d)",
                        delay,
                        attempt + 1,
                        max_retries,
                    )
                ...
                opts = {"skipPreflight": False, "encoding": "base64", "maxRetries": 3}
                ...
                try:
                    ...
                    if "429" in str(e):
                        ...
                        continue
                    logger.error("Failed to send or confirm transaction: %s", e)
                    if attempt == max_retries - 1:
                        return None
                    continue

            except Exception as e:
                logger.warning("Transaction attempt %d failed: %s", attempt + 1, e)
                if attempt == max_retries - 1:
