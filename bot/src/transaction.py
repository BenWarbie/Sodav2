...
from .config import RAYDIUM_AMM_PROGRAM_ID, TOKEN_PROGRAM_ID
...
class TransactionBuilder:
    ...
    def __init__(self, client: AsyncClient, payer: Keypair, pool_details: PoolDetails):
    ...
    def create_swap_instruction(
        ...
        keys = [
            ...
            AccountMeta(
                pubkey=self.pool.token_a_account, is_signer=False, is_writable=True
            ),
            AccountMeta(
                pubkey=self.pool.token_b_account, is_signer=False, is_writable=True
            ),
            AccountMeta(
                pubkey=self.pool.pool_token_mint, is_signer=False, is_writable=True
            ),
            AccountMeta(
                pubkey=self.pool.fee_account, is_signer=False, is_writable=True
            ),
            AccountMeta(
                pubkey=Pubkey.from_string(TOKEN_PROGRAM_ID),
                is_signer=False,
                is_writable=False,
            ),
        ]
    ...
    async def build_sandwich_transactions(
        ...
        try:
            ...
            logger.info("Creating front-run transaction message...")
            front_message = Message.new_with_blockhash(
                [front_run_ix], self.payer.pubkey(), recent_blockhash
            )
            logger.debug(f"Front-run message created: {front_message}")

            # Sign front-run message
            front_signature = self.payer.sign_message(front_message.serialize())
            logger.debug(f"Front-run message signed: {front_signature.hex()}")

            # Create front-run transaction
            front_tx = VersionedTransaction(
                message=front_message, signatures=[front_signature]
            )
            logger.info(f"Created front-run transaction: {front_tx}")

            # Create back-run transaction
            logger.info("Creating back-run transaction...")
            ...
            back_message = Message.new_with_blockhash(
                [back_run_ix], self.payer.pubkey(), recent_blockhash
            )
            logger.debug(f"Back-run message created: {back_message}")

            # Sign back-run message
            back_signature = self.payer.sign_message(back_message.serialize())
            logger.debug(f"Back-run message signed: {back_signature.hex()}")

            # Create back-run transaction
            back_tx = VersionedTransaction(
                message=back_message, signatures=[back_signature]
            )
            logger.info(f"Created back-run transaction: {back_tx}")
